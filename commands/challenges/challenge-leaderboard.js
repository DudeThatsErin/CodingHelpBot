...
module.exports = {
    ...
    async execute (message, args) {
        ...
        const top10 = await connection.all(
            `SELECT author, SUM(CAST(points AS INTEGER)) AS total FROM Submissions WHERE guildId = ? GROUP BY author ORDER BY total DESC LIMIT 10;`,
            [guild]
        );
        ...
        if(top10 === undefined || top10[0] === undefined || top10[0][0] === undefined) {
            ...
            const ponts = await connection.all(
                `SELECT points, SUM(CAST(points AS INTEGER)) AS total FROM Submissions WHERE guildId = ? AND author = ?;`,
                [guild, author]
            );
