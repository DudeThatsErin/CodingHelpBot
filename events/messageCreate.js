...
const { systemManager, challengesDb } = require('../database.js')
...
module.exports = {
    ...
    async execute(message, client) {
        ...
        // Check if challenge system is enabled before querying
        if(command.partsOnly === 1) {
            const challengeSystemEnabled = await systemManager.isSystemEnabled(message.guild.id, 'challenges');
            
            if (!challengeSystemEnabled) {
                message.react('❌');
                message.reply({ content: `The challenge system is not enabled on this server. Please contact a moderator to enable it first.` });
                return;
            }

            try {
                const partsResults = await challengesDb.all(
                ...
                if (partsResults && partsResults.length > 0) {
                    ...
                    for(const participant of partsResults) {
                        if(message.member.id === participant.player) {
                    ...
                    message.react('❌');
                    message.reply({ content: `No participants found in the challenge system. Please contact a challenge moderator.`});
                    return;
                }
            } catch (error) {
                console.error('Error checking challenge participants:', error);
                message.react('❌');
                message.reply({ content: `There was an error checking your participant status. Please try again later.`});
                return;
            }
        }
