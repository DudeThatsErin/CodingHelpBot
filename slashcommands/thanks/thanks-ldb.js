...
module.exports = {
    ...
    async execute (interaction, client) {
        ...
        const top10 = await connection.all(
            `SELECT user, SUM(CAST(thanks AS INTEGER)) AS total FROM Thanks GROUP BY user ORDER BY total DESC LIMIT 10;`
        );
        ...
        if(top10 === undefined || top10[0] === undefined || top10[0][0] === undefined) {
            ...
            const ponts = await connection.all(
                `SELECT thanks, SUM(CAST(thanks AS INTEGER)) AS total FROM Thanks WHERE user = ?;`,
                [author]
            );
